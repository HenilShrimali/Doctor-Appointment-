...
export const notifyNewAppointment = async (appointment) => {
  try {
    ...
    await createAndSendNotification({
      ...
      message: `Your appointment with ${doctorName} on ${appointmentDate} at ${startTime} is confirmed.`,
    ...
    await createAndSendNotification({
      ...
      title: "New Appointment Booked!",
      message: `New appointment with ${doctorName} on ${appointmentDate} at ${startTime}.`,
      type: "new_appointment",
      icon: "calendar",
...
export const notifyAppointmentCancelled = async (appointment, cancelledBy) => {
  try {
    ...
    await createAndSendNotification({
      ...
      recipientId: clinicId,
      title: "Appointment Cancelled",
      message: `Appointment with ${doctorName} on ${appointmentDate} has been cancelled.`,
      type: "appointment_cancelled",
      icon: "x",
      link: "/clinicAppointments",
      data: {
...
export const notifyAppointmentReminder = async (appointment) => {
  try {
    ...
    await createAndSendNotification({
      ...
      message: `Your appointment with ${doctorName} starts at ${startTime}. Please be ready!`,
...
export const notifyConsultationCompleted = async (appointment) => {
  try {
    ...
    await createAndSendNotification({
      ...
      message: `Your consultation with ${doctorName} is complete. ${hasPrescription ? "Your prescription is ready!" : "Check your health records."}`,
...
export const notifyPaymentSuccess = async (appointment, amount) => {
  try {
    ...
    await createAndSendNotification({
      ...
      message: `Payment of ₹${amount} for your appointment with ${doctorName} was successful.`,
